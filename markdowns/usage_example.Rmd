...
The 'blacklist' parameter is a 2 column matrix where the first column is the 'from' and the second is the 'to'. It's important to append the "_t_0" to the names of the nodes, because they are renamed inside the algorithm to that format. A 'whitelist' parameter can also be provided to force some arcs to be present with the same format, but it can sometimes increase the execution time greatly. Only include arcs in t_0 in both the blacklist and whitelist, for now those two are only available for the static network learning, not the transition one.
